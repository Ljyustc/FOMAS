...
def pretrain_formula(formula_enc, formula_pretrain_optimizer):
    ...
    # neg_prob1 = formula_enc.generate_false_formula()
    # neg_prob = torch.cat([neg_prob,neg_prob1],dim=0)
    ...
    pos_loss = loss_func(pos_prob[:,0], pos_label)
    neg_loss = loss_func(neg_prob[:,0], neg_label)
    # loss = loss_func(torch.cat([pos_prob, neg_prob],dim=0)[:,0], labels)
...
def pretrain_formula1(formula_enc, formula_pretrain_optimizer):
    formula_enc.train()
    formula_pretrain_optimizer.zero_grad()
    loss_func = nn.BCELoss(reduction='mean')
    # preciseness + flexibility
    ...
    formula_pretrain_optimizer.step()
    return loss.item()  # , loss_
def pretrain_formula2(formula_enc, formula_pretrain_optimizer):
    formula_enc.train()
    formula_pretrain_optimizer.zero_grad()
    loss_func = nn.BCELoss(reduction='mean')
    # preciseness + flexibility
...
def eval_formula(formula_enc):
    ...
    acc = (pos_acc+neg_acc)/(len(pos_prob)+len(neg_prob))
    return acc
    
def train_tree(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
               encoder, predict, generate, merge, formula_enc, encoder_optimizer, predict_optimizer, generate_optimizer,
    ...
    for t in range(max_target_length):
        ...
        combine_outputs = torch.softmax(outputs,dim=-1) + inher_outputs + select_outputs
        # combine_outputs = torch.softmax(outputs,dim=-1)
        ...
        for idx, l, r, node_stack, i, o, lr, rr, ip in zip(range(batch_size), left_child.split(1), right_child.split(1),
            ...
            if i < num_start:
                node_stack.append(TreeNode(r, rr, inher_prob=ip))
                node_stack.append(TreeNode(l, lr, left_flag=True, inher_prob=ip))
    ...
    loss = masked_cross_entropy(all_node_outputs, target, target_length) + 0.01*masked_cross_entropy_without_logit_mask(all_inher_outputs, target, target_length, inher_masks)+ 0.5 * formula_pre_loss
    # loss = loss_0 + loss_1
    loss.backward()
    # clip the grad
...
def evaluate_tree(input_batch, input_length, generate_nums, encoder, predict, generate, merge, formula_enc, input_lang, output_lang, num_pos, formula_label, text, tokenizer, gt_symbol,
    ...
    encoder_outputs, problem_output = encoder(input_var)
